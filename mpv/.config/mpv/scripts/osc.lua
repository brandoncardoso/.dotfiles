						...
						for _, range in pairs(seekRanges) do
							if range["start"] < pos then
								...
								elem_ass:rect_ccw(pstart,
									elem_geo.h - foV - seekRangeLineHeight, pend,
									elem_geo.h - foV)
							end
						end
					end
				...
				if mouse_hit(element) then
					...
					if not thumbfast.disabled and thumbfast.width ~= 0 and thumbfast.height ~= 0 then
						...
						if osd_w then
							...
							local tooltip_font_size = (user_opts.layout == "box" or user_opts.layout == "slimbox") and
							2 or 12
							local thumb_ty = user_opts.layout ~= "topbar" and
							element.hitbox.y1 - 8 or
							element.hitbox.y2 + tooltip_font_size + 8

							local thumb_pad = 2
							local thumb_margin_x = 20 / r_w
							local thumb_margin_y = (4 + user_opts.tooltipborder) / r_h +
							thumb_pad
							local thumb_x = math.min(
							osd_w - thumbfast.width - thumb_margin_x,
								math.max(thumb_margin_x,
									thumb_tx / r_w - thumbfast.width / 2))
							local thumb_y = user_opts.layout ~= "topbar" and
							thumb_ty / r_h - thumbfast.height - tooltip_font_size / r_h -
							thumb_margin_y or thumb_ty / r_h + thumb_margin_y
